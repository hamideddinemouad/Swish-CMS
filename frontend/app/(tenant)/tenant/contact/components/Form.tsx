    ...
    <section className="space-y-5 py-8">
      ...
      <form className="space-y-4">
        {fields.map((field) => (
          <label
            key={field.label}
            className={`block space-y-2 text-sm ${tokens.theme.text.secondary}`}
          ...
          >
            ...
            {field.type === "textarea" ? (
              <textarea
                ...
                rows={6}
                className={`${tokens.cards.compact} min-h-[160px] w-full border-[1px] p-4 text-sm ${tokens.theme.text.primary}`}
              />
            ) : (
              <input
                required={field.required}
                type={field.type}
                placeholder={field.placeholder}
                className={`${tokens.cards.compact} w-full border-[1px] p-4 text-sm ${tokens.theme.text.primary}`}
              />
            )}
          </label>
        ))}
